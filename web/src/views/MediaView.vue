<template>
  <div class="space-y-6 ">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
      <div>
        <h1 class="text-2xl font-bold text-slate-950">{{ t('media.title') }}</h1>
        <p class="mt-1 text-sm text-slate-500">{{ t('media.description') }}</p>
      </div>
      <div class="flex flex-wrap items-center gap-2">
        <button type="button" class="btn-secondary shrink-0" :disabled="regeneratingThumbs"
          @click="regenerateThumbnails">
          <Icon :icon="regeneratingThumbs ? 'mdi:loading' : 'mdi:image-refresh-outline'" class="h-4 w-4"
            :class="{ 'animate-spin': regeneratingThumbs }" />
          {{ regeneratingThumbs ? t('media.regeneratingThumbs') : t('media.regenerateThumbs') }}
        </button>
        <button type="button" class="btn-primary shrink-0" @click="fileInput.click()">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-3 lg:grid-cols-4">
      ...
      <MediaCategorySidebar v-model="selectedCategory" :categories="categories" :stats-files="statsFiles"
        :dragged-file="draggedFile" :selected-count="selectedCount" :selected-names="selectedNames"
      ...
      <section class="card relative min-w-0 p-5 col-span-2 lg:col-span-3" @dragenter.prevent="onContentDragEnter"
        @dragleave="onContentDragLeave" @dragover.prevent @drop.prevent="onContentDrop">
        <div class="mb-5 flex flex-col gap-4">
          <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 class="text-base font-semibold text-slate-900">{{ t('media.filesTitle') }}</h2>
              <p class="mt-1 text-xs text-slate-500">
                {{ selectedCategoryLabel }} · {{ totalFiles }} {{ mediaFileLabel(totalFiles) }}
              </p>
            </div>
            <div class="flex items-center gap-2 self-start sm:self-auto">
              <button type="button" class="rounded-lg border p-2 transition-colors"
                ...
                :title="t('media.gridView')" @click="viewMode = 'grid'">
              ...
              <button type="button" class="rounded-lg border p-2 transition-colors"
                ...
                :title="t('media.listView')" @click="viewMode = 'list'">
                <Icon icon="mdi:view-list-outline" class="h-5 w-5" />
              </button>
            </div>
          </div>

          <div class="grid gap-3 lg:grid-cols-[minmax(14rem,1fr)_10rem_10rem_10rem]">
            <label class="relative block">
              <span class="sr-only">{{ t('media.search') }}</span>
              <Icon icon="mdi:magnify"
                class="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
              <input id="media-search" v-model="search" type="search" :placeholder="t('media.searchPlaceholder')"
                class="form-input w-full rounded-lg border-slate-300 pl-9 text-sm" />
            </label>
            <label class="block">
              <span class="sr-only">{{ t('media.filterType') }}</span>
              <select v-model="mediaType" class="form-select w-full rounded-lg border-slate-300 text-sm">
            ...
            <label class="block">
              ...
              <select v-model="sortOrder" class="form-select w-full rounded-lg border-slate-300 text-sm">
            ...
            <label class="block">
              ...
              <select v-model="usageFilter" class="form-select w-full rounded-lg border-slate-300 text-sm">
                <option v-for="option in usageFilterOptions" :key="option.value" :value="option.value">
                  {{ option.label }}
                </option>
              </select>
            </label>
        ...
        <div v-if="uploadError" class="mb-4 rounded-lg bg-red-50 p-3 text-sm text-red-700">{{ uploadError }}</div>

        <Transition v-bind="ht">
          <div v-if="selectedCount > 0"
            class="mb-4 rounded-lg border border-theme-200 bg-theme-50/60 px-3 py-3 shadow-sm">
            <MediaBulkEditBar :categories="categoryTree" :selected-count="selectedCount"
              :all-page-selected="allPageSelected" :page-count="files.length" :applying="bulkWorking"
              @apply="onBulkApply" @delete-selected="confirmBulkDelete" @clear-selection="clearSelection"
        ...
        <div v-if="!loading && files.length === 0"
          ...
          :class="isDraggingFile ? 'border-theme-400 bg-theme-50' : 'border-slate-300 hover:border-theme-400'"
          @click="fileInput.click()">
          <Icon class="mx-auto mb-2 h-8 w-8 transition-colors"
            :class="isDraggingFile ? 'text-theme-500' : 'text-slate-400'" icon="mdi:cloud-upload-outline" />
          <p class="text-sm text-slate-600">
            <span class="font-medium text-theme-600">{{ t('media.clickUpload') }}</span> {{ t('media.dragDrop') }}
          </p>
          <p class="mt-1 text-xs text-slate-400">
            {{ uploading ? t('media.uploading') : uploadHint }}
          </p>
          <p v-if="search.trim() !== '' || mediaType !== 'all'" class="mt-2 text-xs text-slate-400">
            {{ t('media.hiddenByFilters') }}
          </p>
        </div>

        <Transition name="drop-overlay">
          <div v-if="isDraggingFile && files.length > 0"
        ...
        <template v-else-if="files.length > 0">
          <div v-if="totalPages > 1"
            ...
            <p>{{ t('media.showing', { start: pageStart, end: pageEnd, total: totalFiles }) }}</p>
            <div class="flex items-center gap-2">
              ...
              <span class="min-w-20 text-center">{{ t('media.pageOf', { page: currentPage, total: totalPages })
              }}</span>
              <button type="button"
                class="btn-secondary px-3 py-1.5 text-sm disabled:cursor-not-allowed disabled:opacity-50"
          ...
          <div v-if="viewMode === 'grid'" class="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5">
            <article v-for="(file, fileIndex) in files" :key="file.name"
              class="group relative cursor-grab rounded-lg border bg-white p-2 transition-all hover:border-theme-300 hover:shadow-sm"
              :class="{ 'opacity-50': draggedFile?.name === file.name, 'ring-2 ring-theme-500': isSelected(file) }"
              draggable="true" @dragstart="onFileDragStart(file, $event)" @dragend="onFileDragEnd">
              <label
                ...
                :class="selectionMode ? 'opacity-100' : 'pointer-events-none opacity-0 group-hover:pointer-events-auto group-hover:opacity-100 focus-within:pointer-events-auto focus-within:opacity-100'">
                <span class="sr-only">{{ t('media.selectFile', { name: file.name }) }}</span>
                <input type="checkbox" class="form-checkbox rounded border-slate-300 text-theme-600"
                  :checked="isSelected(file)" @click.stop="onCheckboxClick(file, fileIndex, $event)" />
              </label>
              ...
              <button type="button"
                ...
                :title="t('media.delete')" @click.stop="confirmDelete(file)">
              ...
              <button type="button"
                ...
                <img v-if="isImage(file.name)" :src="mediaPreviewUrl(file)" class="h-full w-full object-cover"
                  :alt="file.name" />
              ...
              <p class="text-xs text-slate-400 flex items-center gap-1">
                <Icon v-if="file.visibility === 'private'" icon="mdi:lock-outline"
                  class="h-3 w-3 shrink-0 text-amber-500" :title="t('media.private')" />
          ...
          <div v-else class="overflow-hidden rounded-lg border border-slate-200">
            <div v-for="(file, fileIndex) in files" :key="file.name"
              ...
              :class="{ 'opacity-50': draggedFile?.name === file.name, 'ring-2 ring-inset ring-theme-500': isSelected(file) }"
              draggable="true" @dragstart="onFileDragStart(file, $event)" @dragend="onFileDragEnd">
              ...
              <div class="min-w-0">
                <button type="button"
                  ...
                  :title="file.name" @click="openDetail(file)">
                  {{ file.name }}
                </button>
              ...
              <div class="flex items-center gap-2 sm:justify-end">
                <button type="button" class="btn-danger px-2 py-1 text-xs" :title="t('media.delete')"
          ...
          <div v-if="totalPages > 1"
            ...
            <p>
              {{ t('media.showing', { start: pageStart, end: pageEnd, total: totalFiles }) }}
            </p>
            <div class="flex items-center gap-2">
              <button type="button"
                class="btn-secondary px-3 py-1.5 text-sm disabled:cursor-not-allowed disabled:opacity-50"
    ...
    <!-- Media detail panel -->
    <SlidePanel v-model="showDetail" :title="detailFile?.name ?? ''"
      :subtitle="detailFile ? formatBytes(detailFile.size) : ''" width="28rem">
      <div v-if="detailFile" class="p-6 space-y-6">
        ...
        <div class="rounded-xl overflow-hidden bg-slate-100 flex items-center justify-center">
          <img v-if="isImage(detailFile.name)" :src="detailFile.url" class="max-w-full max-h-80 object-contain"
            :alt="detailFile.name" />
          <div v-else class="py-16 flex flex-col items-center gap-2 text-slate-400">
        ...
        <dl class="space-y-3 text-sm">
          <div>
            <dt class="text-xs font-medium text-slate-400 uppercase tracking-wider mb-1">{{ t('media.fileName') }}</dt>
            <dd v-if="renamingFile" class="space-y-2">
              <input v-model="renameFileName" type="text" class="form-input w-full rounded-lg border-slate-300 text-sm"
                @keydown.enter.prevent="executeRenameFile" @keydown.esc="cancelRenameFile" />
              <div class="grid gap-2 sm:grid-cols-2">
                <button type="button" class="btn-primary justify-center text-sm" :disabled="renamingFileWorking"
                  @click="executeRenameFile">
                  <Icon icon="mdi:check" class="h-4 w-4" />
                  {{ t('media.save') }}
                </button>
                <button type="button" class="btn-secondary justify-center text-sm" :disabled="renamingFileWorking"
            ...
            <dd v-else class="flex items-start justify-between gap-3">
              <button type="button" class="min-w-0 break-all text-left text-slate-800 hover:text-theme-700"
                :title="t('media.doubleClickRename')" @dblclick="startRenameFile(detailFile)">
                {{ detailFile.name }}
              </button>
              <button type="button" class="btn-secondary shrink-0 px-2 py-1 text-xs" :title="t('media.renameFile')"
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              <dt class="text-xs font-medium text-slate-400 uppercase tracking-wider mb-1">{{ t('media.size') }}</dt>
              <dd class="text-slate-800">{{ formatBytes(detailFile.size) }}</dd>
            </div>
            <div>
              <dt class="text-xs font-medium text-slate-400 uppercase tracking-wider mb-1">{{ t('media.type') }}</dt>
              <dd class="text-slate-800">{{ detailFile.mime ?? fileTypeLabel(detailFile) }}</dd>
            </div>
          </div>
          <div v-if="imageDimensions(detailFile)">
            <dt class="text-xs font-medium text-slate-400 uppercase tracking-wider mb-1">{{ t('media.dimensions') }}
            </dt>
            <dd class="text-slate-800">{{ imageDimensions(detailFile) }}</dd>
          </div>
          <div v-if="detailFile.created_at">
            <dt class="text-xs font-medium text-slate-400 uppercase tracking-wider mb-1">{{ t('media.uploaded') }}</dt>
            <dd class="text-slate-800">{{ formatDate(detailFile.created_at) }}</dd>
          </div>
          <div v-if="detailFile.uploaded_at || detailFile.uploaded_by">
            <dt class="text-xs font-medium text-slate-400 uppercase tracking-wider mb-1">{{ t('media.uploadedBy') }}
            </dt>
            <dd>
              <template v-if="userMap[detailFile.uploaded_by]">
                <div class="flex items-center gap-2">
                  <div
                    ...
                    <img v-if="userMap[detailFile.uploaded_by].has_avatar"
                      :src="`/admin/api/users/${detailFile.uploaded_by}/avatar`" class="w-full h-full object-cover"
                      :alt="userMap[detailFile.uploaded_by].username" />
                    <span v-else>{{ userMap[detailFile.uploaded_by].username?.[0]?.toUpperCase() }}</span>
                  </div>
                  <span class="text-sm text-slate-800">{{ userMap[detailFile.uploaded_by].username }}</span>
                  <span v-if="detailFile.uploaded_at" class="text-xs text-slate-400">{{
                    formatDate(detailFile.uploaded_at) }}</span>
                </div>
              </template>
              <span v-else class="text-slate-500 text-sm">{{ detailFile.uploaded_by ?? '—' }}</span>
            </dd>
          </div>
        </dl>
        ...
        <!-- Used in -->
        <div>
          <label class="text-xs font-medium text-slate-400 uppercase tracking-wider block mb-2">{{ t('media.usedIn')
            }}</label>
          <p v-if="detailFileUsages.length === 0" class="text-sm text-slate-400">{{ t('media.notUsed') }}</p>
          <ul v-else class="space-y-1.5">
            <li v-for="ref in detailFileUsages" :key="`${ref.collection}:${ref.id}`"
              class="flex items-start justify-between gap-2 rounded-lg border border-slate-100 bg-slate-50 px-3 py-2 text-sm">
              <span class="min-w-0">
                <RouterLink :to="`/content/${ref.collection}/${ref.id}`"
                  class="block truncate font-medium text-slate-800 hover:text-theme-700">{{ ref.title }}</RouterLink>
                <span class="text-xs text-slate-400">{{ ref.collection }}</span>
              </span>
              <Icon icon="mdi:arrow-top-right" class="mt-0.5 h-4 w-4 shrink-0 text-slate-300" />
            </li>
          </ul>
        </div>
        ...
        <!-- Category -->
        <div>
          <label class="text-xs font-medium text-slate-400 uppercase tracking-wider block mb-1">{{ t('media.category')
          }}</label>
          <select :value="detailFile.category ?? ''" class="form-select w-full rounded-lg border-slate-300 text-sm"
            @change="onDetailCategoryChange($event.target.value)">
            <option value="">{{ t('media.noCategory') }}</option>
            <option v-for="cat in categoryTree" :key="cat.path" :value="cat.path">{{ cat.optionLabel }}</option>
          </select>
        </div>

        <!-- Visibility -->
        <div>
          <label class="text-xs font-medium text-slate-400 uppercase tracking-wider block mb-1">{{ t('media.visibility')
          }}</label>
          <select :value="detailFile.visibility ?? 'public'"
            ...
            <option value="private">{{ t('media.privateRequiresToken') }}</option>
          </select>
        </div>
        ...
        <div class="space-y-3">
          <div>
            <label :for="`detail-alt-${detailFile.name}`"
              class="text-xs font-medium text-slate-400 uppercase tracking-wider block mb-1">{{ t('media.altText')
              }}</label>
            <input :id="`detail-alt-${detailFile.name}`" v-model="detailAlt" type="text"
              class="form-input w-full rounded-lg border-slate-300 text-sm" :placeholder="t('media.altPlaceholder')"
              @blur="saveMetaIfChanged" />
          </div>
          <div>
            <label :for="`detail-title-${detailFile.name}`"
              class="text-xs font-medium text-slate-400 uppercase tracking-wider block mb-1">{{ t('media.titleField')
              }}</label>
            <input :id="`detail-title-${detailFile.name}`" v-model="detailTitle" type="text"
              class="form-input w-full rounded-lg border-slate-300 text-sm" :placeholder="t('media.titlePlaceholder')"
              @blur="saveMetaIfChanged" />
          </div>
          <div v-if="savingMeta" class="text-xs text-slate-400">{{ t('media.saving') }}</div>
        </div>

        <!-- URL -->
        <div>
          <label class="text-xs font-medium text-slate-400 uppercase tracking-wider block mb-1">{{ t('media.url')
          }}</label>
          <div class="flex gap-2">
            <input :value="origin + detailFile.url" readonly
              class="form-input flex-1 rounded-lg border-slate-300 text-sm bg-slate-50 text-slate-600" />
            <button type="button" class="btn-secondary shrink-0" @click="copyUrl(origin + detailFile.url)">{{
              t('media.copy') }}</button>
          </div>
        </div>
        ...
        <div class="pt-2 border-t border-slate-100">
          <button type="button" class="btn-danger w-full" @click="confirmDelete(detailFile)">
            {{ t('media.deleteFile') }}
          </button>
        </div>
      </div>
    ...
    <ConfirmModal v-model="showDeleteModal" :title="t('media.deleteFileTitle')"
      :message="t('media.deleteFileMessage', { name: deleteTarget?.name })" :confirm-label="t('media.delete')"
      :loading="deleting" @confirm="executeDelete" />

    <ConfirmModal v-model="showBulkDeleteModal" :title="t('media.deleteSelectedTitle')"
      :message="t('media.deleteSelectedMessage', { count: selectedCount, itemLabel: mediaFileLabel(selectedCount) })"
      :confirm-label="t('media.deleteSelected')" :loading="bulkWorking" @confirm="executeBulkDelete" />
...
const viewMode = ref(
  typeof localStorage !== 'undefined' && localStorage.getItem('mediaViewMode') === 'list'
    ? 'list'
    : 'grid'
)
...
async function loadUsers() {
  try { users.value = (await api.users.list()).data ?? [] } catch { /* non-admin, ignore */ }
}
...
const detailFileUsages = computed(() => (detailFile.value ? (usages.value[detailFile.value.name] ?? []) : []))
const uploadHint = computed(() => selectedCategory.value === null ? t('media.anyAllowedType') : t('media.uploadsTo', { category: selectedCategory.value }))
...
const categoryTree = computed(() => categories.value.map((category) => {
  const parts = categoryParts(category)
...
async function load() {
  ...
  try {
    ...
    const isVisibilityFilter = usageFilter.value === 'public' || usageFilter.value === 'private'
    ...
    if (isUnusedFilter) {
      const filtered = (res.data ?? []).filter((file) => !usages.value[file.name]?.length)
...
async function uploadFiles(fileList) {
  ...
  try {
    ...
    toast.success(t('media.uploadedToast', { count: uploadedCount, itemLabel: mediaFileLabel(uploadedCount) }))
    await loadStats()
    await load()
  } catch (err) {
...
async function saveMetaIfChanged() {
  ...
  try {
    ...
    files.value = files.value.map((item) => item.name === file.name ? updated : item)
...
async function executeRenameFile() {
  ...
  try {
    ...
    files.value = files.value.map((item) => item.name === file.name ? renamed : item)
...
async function executeDelete() {
  ...
  try {
    ...
    selectedMedia.value = new Set(selectedNames.value.filter((name) => name !== file.name))
...
function onCheckboxClick(file, fileIndex, event) {
  ...
  if (event.shiftKey && lastSelectedIndex.value >= 0 && lastSelectedIndex.value !== fileIndex) {
...
async function updateFilesCategory(fileNames, category, clearAfterUpdate = false) {
  ...
  try {
    ...
    const updatedByName = new Map((res.data ?? []).map((file) => [file.name, file]))
    categories.value = res.meta?.categories ?? res.categories ?? categories.value
    files.value = files.value.map((file) => updatedByName.get(file.name) ?? file)
    if (detailFile.value) {
      detailFile.value = files.value.find((file) => file.name === detailFile.value?.name) ?? detailFile.value
    }
    toast.success(t('media.updatedToast', { count: updatedByName.size, itemLabel: mediaFileLabel(updatedByName.size) }))
...
async function executeBulkDelete() {
  ...
  try {
    ...
    selectedMedia.value = new Set(selectedNames.value.filter((name) => !deleted.has(name)))
    if (detailFile.value && deleted.has(detailFile.value.name)) showDetail.value = false
    showBulkDeleteModal.value = false
    toast.success(t('media.deletedToast', { count: deleted.size, itemLabel: mediaFileLabel(deleted.size) }))
    await loadStats()
    await load()
  } catch (err) {
    toast.error(err.message)
...
async function onDetailCategoryChange(category) {
  ...
  detailFile.value = files.value.find((f) => f.name === detailFile.value?.name) ?? detailFile.value
}
...
async function updateFileCategory(file, category) {
  try {
    ...
    categories.value = res.meta?.categories ?? res.categories ?? categories.value
    files.value = files.value.map((item) => item.name === file.name ? res.data : item)
    await loadStats()
    await load()
  } catch (err) {
    toast.error(err.message)
...
async function updateFilesVisibility(fileNames, visibility, clearAfterUpdate = false) {
  const names = Array.from(new Set(fileNames.filter(Boolean)))
  if (names.length === 0) return
...
async function onCategoryFileDrop({ file, targetCategory }) {
  ...
  if (selectedCount.value > 0) {
    await updateFilesCategory([...selectedNames.value, file.name], targetCategory, true)
    return
  }
